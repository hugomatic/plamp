...
The adjustment matrix stacks three separately cut coupons in each column. Its final CSG operation must unite the matrix body first and then reapply each cell's shared catcher negative to that united body. Otherwise the coupon above fills the portion of the lower coupon's pointy roof that crosses their common boundary: the top row remains pointy, but the lower rows acquire flat floating ceilings. The catcher negative already extends beyond the exterior tunnel mouth; changing its entry direction or tunnel length does not address this stacked-body boundary.
