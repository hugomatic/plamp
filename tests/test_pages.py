...
class PageRenderTests(unittest.TestCase):
    ...
    def test_timer_dashboard_renders_binary_controller_health(self):
        ...
        self.assertIn("function controllerStreamTone(role, health) {", html)
        self.assertIn("const STREAM_WARN_AFTER_SECONDS = 5;", html)
        self.assertIn("const STREAM_ERROR_AFTER_SECONDS = 10;", html)
        self.assertIn('status.className = tone === "ok" ? "editor-success" : tone === "warn" ? "editor-warn" : "editor-error";', html)
        self.assertIn('`OK — last verified ${formatAge(health?.last_verified_at || health?.checked_at)}`', html)
        self.assertIn('`ERROR: ${health?.error?.message || "controller status stream disconnected"}`', html)
        self.assertIn("last verified", html)
        self.assertIn(".editor-warn {", html)
        ...
        self.assertNotIn('error: {message: "controller status stream disconnected"}', html)
    ...
    def test_timer_dashboard_renders_effective_schedule_traces(self):
        ...
        self.assertIn('const TIMER_HISTORY_HORIZONS = [["last hour", 3600], ["last 24h", 86400], ["last 30 days", 30 * 86400]];', html)
